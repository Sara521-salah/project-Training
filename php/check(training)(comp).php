...
if ( $_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if( empty($_POST['Email_comp']) ==  true || empty($_POST['Password_comp']) ==  true){
        ...
        for( $i = 0 ; $i < $resLenght ; $i++){
                ...
                header("location:../pages/training(log).php");
                exit();
            }    
        }
        for( $x = 0 ; $x < $resLenght ; $x++){
            if ($_POST["Email_comp"] != $res[$x]['email'] && $_POST["Password_comp"] == $res[$x]['password']){
                header("location:../pages/training.php?emailErrcomp= Wrong Email!");
                exit();
             }   
         } 
         for( $y = 0 ; $y < $resLenght ; $y++){
             if ($_POST["Email_comp"] == $res[$y]['email'] && $_POST["Password_Password_comptrainee"] != $res[$y]['password']){
                 header("location:../pages/training.php?passErrcomp= Wrong Password!");
                 exit();
             }   
         } 
